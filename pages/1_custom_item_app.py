...
def diff_pct_cell_style(val):
    try:
        ...
        if numeric > 0:
            return "background-color: #e8f5e9; color: #256029;"
        return ""
    except Exception:
        return ""
...
def build_item_export_df(df, for_excel=False):
    ...
    if not for_excel:
        formatters = {
            ...
            "Diff %": lambda x: f"{x:.1%}" if pd.notna(x) else "",
        }
...
def build_item_style_map(df):
    ...
    for row_idx in range(len(df)):
        for col_idx, col_name in enumerate(df.columns):
            ...
            if col_name in ["Day School Attainment", "School-based Expected Attainment"]:
                style_map[(row_idx, col_idx)] = {
                    "fill": {
                        "High attainment": "#e8f5e9",
                        "Intermediate attainment": "#e3f2fd",
                        "Low attainment": "#fff8e1",
                        "達到校本預期 | Attained": "#d0f0c0",
                        "低於校本預期，建議關注 | Below Expectation": "#ffebee",
                    }.get(value, "")
                }
                if value == "達到校本預期 | Attained":
                    style_map[(row_idx, col_idx)].update({"font_color": "#256029", "bold": True})
                elif value == "低於校本預期，建議關注 | Below Expectation":
                    style_map[(row_idx, col_idx)].update({"font_color": "#b71c1c", "italic": True})
    return style_map
...
if not df_item_c.empty:
    ...
    with left_col:
        ...
        df_display["Diff"] = df_display["Your school Mean"] - df_display["Day schools Mean"]
        df_display["Diff %"] = df_display["Your school Mean %"] - df_display["Day schools Mean %"]
        if "Day School Attainment" in df_display.columns:
            cols = [c for c in df_display.columns if c not in ["Diff", "Diff %"]]
            insert_pos = cols.index("Day School Attainment")
            cols = cols[:insert_pos] + ["Diff", "Diff %"] + cols[insert_pos:]
            df_display = df_display[cols]

        base_count_high = int((df_display["Day School Attainment"] == "High attainment").sum())
        base_count_inter = int((df_display["Day School Attainment"] == "Intermediate attainment").sum())
        base_count_low = int((df_display["Day School Attainment"] == "Low attainment").sum())
        base_count_attained = int((df_display["School-based Expected Attainment"] == "達到校本預期 | Attained").sum())
        base_count_below = int((df_display["School-based Expected Attainment"] == "低於校本預期，建議關注 | Below Expectation").sum())
        base_count_below_high = int(((df_display["Day School Attainment"] == "High attainment") & (df_display["School-based Expected Attainment"] == "低於校本預期，建議關注 | Below Expectation")).sum())

        st.markdown("**📊 2.3 校本分析摘要 | Section 2 Summary**")
        kpi_main_cols = st.columns([1, 1])
        with kpi_main_cols[0]:
            kpi_cols_level = st.columns(3)
            kpi_cols_level[0].metric("High attainment 題數", base_count_high)
            kpi_cols_level[1].metric("Intermediate attainment 題數", base_count_inter)
            kpi_cols_level[2].metric("Low attainment 題數", base_count_low)
        with kpi_main_cols[1]:
            kpi_cols_expected = st.columns(2)
            kpi_cols_expected[0].metric("Attained 題數", base_count_attained)
            kpi_cols_expected[1].metric("Below Expectation 題數", base_count_below)

        review_text = "目前校本預期分析結果已完成。"
        if base_count_below:
            review_text += f" 其中 {base_count_below} 題未達校本預期。"
            if base_count_below_high:
                review_text += f" 有 {base_count_below_high} 題屬於高得分率題目，建議優先關注。"
        else:
        ...
        for col, filter_mask in active_filters.items():
            if isinstance(filter_mask, pd.Series):
        ...
        if filter_summary_parts:
            st.caption(f"✓ {' | '.join(filter_summary_parts)}")
        else:
        ...
        try:
            ...
            for idx, lvl in enumerate(st.session_state.item_sort_levels):
                ...
                if col == "Day School Attainment":
                    ...
                    final_df[temp_col] = final_df[col].map({"High attainment": 3, "Intermediate attainment": 2, "Low attainment": 1}).fillna(0)
                    ...
                    temp_col = f"___item_sort_key_{idx}"
                    final_df[temp_col] = final_df[col].map({"低於校本預期，建議關注 | Below Expectation": 1, "達到校本預期 | Attained": 2}).fillna(0)
                    sort_by_list.append(temp_col)
                    ascending_list.append(asc)
                    temp_sort_cols.append(temp_col)
            ...
            final_df = final_df.drop(columns=[c for c in temp_sort_cols if c in final_df.columns])
        except Exception:
            pass
    ...
    with right_col:
        ...
        st.session_state["custom_item_overview_df"] = final_df.copy()
        ...
        st.dataframe(
            final_df.style
                ...
                .map(diff_pct_cell_style, subset=["Diff %"]),
            height=3000,
            use_container_width=True,
            hide_index=True
        )

        overview_export_df = build_item_export_df(final_df, for_excel=True)
        overview_export_pdf = convert_df_to_pdf(build_item_export_df(final_df, for_excel=False), build_item_style_map(final_df), title="項目分析 | 總覽表 Item Analysis | Overview Table")
        overview_export_excel = convert_df_to_styled_excel(overview_export_df, build_item_style_map(final_df), sheet_name="Item Overview")
