...
if not df_item_c.empty:
    ...
    # Add custom diff columns before Day School Attainment
    ...
    def build_item_export_df(df, for_excel=False):
        ...
        if not for_excel:
            formatters = {
                ...
                "Diff": lambda x: f"{x:.1f}" if pd.notna(x) else "",
    ...
    with right_col:
        total_filtered = len(final_df)
        count_high = int((final_df["Day School Attainment"] == "High attainment").sum())
        count_inter = int((final_df["Day School Attainment"] == "Intermediate attainment").sum())
        count_low = int((final_df["Day School Attainment"] == "Low attainment").sum())
        count_attained = int((final_df["School-based Expected Attainment"] == "達到校本預期 | Attained").sum())
        count_below = int((final_df["School-based Expected Attainment"] == "低於校本預期，建議關注 | Below Expectation").sum())
        count_below_high = int(((final_df["Day School Attainment"] == "High attainment") & (final_df["School-based Expected Attainment"] == "低於校本預期，建議關注 | Below Expectation")).sum())
